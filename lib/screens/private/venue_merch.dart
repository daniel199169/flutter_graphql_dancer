...
class VenueMerch extends StatefulWidget {
  @override
  _VenueMerchState createState() => _VenueMerchState();
}

class _VenueMerchState extends State<VenueMerch> with LoadingMixin<VenueMerch> {
  ...
  Future<void> load() async {
    ...
    if (r.hasException) {
      print("venue_merch: error fetching: ${r.exception.graphqlErrors[0].message}");
                  ...
                  child: Row(
                    children: <Widget>[
                      ...
                      Padding(
                        ...
                        child: Text(
                          'Opening hours: ${venue.openingHours ?? "N/A"}',
                          style: TextStyle(fontSize: 16, color: Colors.black),
                        ),
                      ),
                    ],
  ...
  Widget build(BuildContext context) {
    ...
    final separator = Container(height: 20, color: Colors.white,);
    ...
    if (loading) {
      ...
      print("venue_merch: error loading venues: $error");
      children.add(Center(child: Text("Error loading data")));
    } else {
      for (var location in locations) {
        if (location.category == "merch") {
